function [x, iter, info] = LMRLRalgorithm_mri(A, m, xSz, param)
%LMRLRALGORITHM_MRI Computes the Local MultiResolution Low-Rank regularization
%algorithm given MRI operator A, data m and additional parameters param
%
% INPUTS
%   A       MRI forward operator in Emat_xyz class
%   m       k-Data stack
...
while iter < maxIter
    ...
    % PDFP steps
    y = x - gamma*reshape(bp,xSz) - lambda*invBv; % 3D array
    ...
    x = xOld - gamma*bp - lambda*invBv; % 3D array
    ...
    if mod(iter,plotFreq) == 0
        figure(100)
        montage(abs(x), 'DisplayRange', []);
        title(sprintf('Reconstruction at iter: %d', iter));
        drawnow

        figure(101)
        imagesc(abs(prox2(:,:,1,1)));
